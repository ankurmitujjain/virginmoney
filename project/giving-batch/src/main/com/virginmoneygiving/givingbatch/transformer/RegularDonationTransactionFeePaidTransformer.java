 ...
 * @author Tarun Adiwal
 */
public class RegularDonationTransactionFeePaidTransformer implements
        StepExecutionListener, ItemProcessor<Payment, TransactionFeePaid> {

    /** Create instance of Logger to show Log information. */
    private static Logger LOGGER =
            Logger
                    .getLogger(RegularDonationTransactionFeePaidTransformer.class);
    ...
    public TransactionFeePaid process(Payment payment) throws GivingBatchException {
        if(LOGGER.isTraceEnabled()) {
            LOGGER.trace("RegularDonationTransactionFeePaidTransformer - Start");
        }
        ...
        if (payment != null) {
            ...
            for (HandlingCharge handlingChargeIt : handlingChargeSet) {
                ...
                feePaid.setCreditNoteNumber(handlingChargeIt
                        .getFinanceReference());
                feePaid
                        .setInvoiceNumber(handlingChargeIt
                                .getFinanceReference());
                ...
                String batchNumber =
                        (String) context.get(Constant.BATCH_NUMBER);
                ...
                try {
                    ...
                    LOGGER
                            .error(
                                    "Error occurred while creating batch Handling Charge Entity :" +
                                    " RegularDonationTransactionFeePaidTransformer."
                    ...
                    LOGGER
                            .error("Error occured while calling creating batch Handling Charge Entity "
                ...
                for (VatAmount vatAmountIterator : vatAmountList) {
                    ...
                    try {
                        ...
                        LOGGER
                                .error(
                                        "Error occurred while creating batch Handling Charge VAT Entity : "
                                                + "RegularDonationTransactionFeePaidTransformer."
                                                + batchExtManagementServiceFaultException.getMessage(),batchExtManagementServiceFaultException);
                        throw new GivingBatchException(LOGGER,  "Batch Number"+batchNumber, batchExtManagementServiceFaultException);
                    }
                        ...
                        LOGGER
                                .error("Error occured while calling creating batch Handling Charge VAT Entity "
                                        + exception.getMessage(),exception);
                        throw new GivingBatchException(LOGGER,  "Batch Number"+batchNumber, exception);
                    }
                }
        ...
        if(LOGGER.isTraceEnabled()) {
            LOGGER.trace("RegularDonationTransactionFeePaidTransformer - End");
        }
        return feePaid;
    }
    ...
    private void updateObjectInContext(TransactionFeePaid transactionFeePaid) {
        LOGGER
                .trace("Inside RegularDonationTransactionFeePaidTransformer UpdateObjectContext -Start");
        ...
        if (transactionFeePaidList == null) {
            ...
            executionContext.put(
                    Constant.REGULAR_DONATION_TRANSACTION_FEE_PAID_OBJECT_LIST,
                    transactionFeePaidList);
        }
        transactionFeePaidList.add(transactionFeePaid);
        LOGGER
                .trace("Inside RegularDonationTransactionFeePaidTransformer UpdateObjectContext -End.");
    }
    ...
    public void beforeStep(StepExecution stepExecution) {
        ...
        String batchNumber = Util.getGeneratedBatchNumber(
                MasterDataCodeConstants.BatchEventType.REGULAR_DONATION_TRANSACTION_FEE_PAID.getCode());
        ServiceBatch serviceBatch = Util.createServiceBatch(batchNumber, stepExecution);
        serviceBatch.setCurrentJobName("regularDonationtransactionFeePaidJob");
        serviceBatch.setNextBatchName("TRANSACTION_FEE_PAID");
        ...
        try {
            ...
            LOGGER.error("Error occurred while creating batch  : RegularDonationTransactionFeePaidTransformer."
                        + batchExtManagementServiceFaultException.getMessage(), batchExtManagementServiceFaultException);
            throw new GivingBatchException(LOGGER,  "Batch Number"+batchNumber, batchExtManagementServiceFaultException);
        }
        catch(Exception exception)
