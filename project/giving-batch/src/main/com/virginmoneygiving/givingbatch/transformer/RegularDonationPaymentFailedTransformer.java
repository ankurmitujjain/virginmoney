...
/**
 * This class takes the Payment object and transform it to FailedPayment
 ...
 */
public class RegularDonationPaymentFailedTransformer implements
		StepExecutionListener, ItemProcessor<Payment, FailedPayment> {
	
	/** Create instance of Logger to show Log information. */
	private static Logger LOGGER = Logger
			.getLogger(RegularDonationPaymentFailedTransformer.class);
...
//						.getTransactionAmount());
		...
		batchEntity
				.setNextBatchStatus(MasterDataCodeConstants.WebDonation.WEB_DONATION_PAYMENT_FAILED
						.getCode());
		...
		try {
            ...
            LOGGER.error("Error occurred while creating batch Payment Entity : RegularDonationPaymentFailedTransformer."
	...
	public String getGeneratedInvoiceNumber(PaymentService paymentService) {
		String generateSequesnceNumber = paymentService.generateSequence(
				"WEB_DONATION", "CREDIT_NOTE");
		String referenceNumber = generateSequenceHelper.prefixReferenceType(
				"WEB_DONATION", "CREDIT_NOTE", generateSequesnceNumber);
     ...
     * 
     * @param failedPayment of type FailedPayment to add this object to
     * executionContextList.
     */
	private void updateObjectInContext(FailedPayment failedPayment) {

		LOGGER
				.trace("Inside PaymentInitiatedTransformer UpdateObjectContext -Start");
		if (executionContext != null) {
		...
		/*
		 * LOGGER.trace(
		 * "Inside RegularDonationPaymentFailedTransformer UpdateObjectContext -Start"
		 * ); ExecutionContext executionContext =
		 * stepExecution.getJobExecution().getExecutionContext();
		 * List<FailedPayment> paymentFailedObjectList = (List<FailedPayment>)
		 * executionContext
		 * .get(Constant.REGULAR_DONATION_PAYMENT_FAILED_OBJECT_LIST); if
		 * (paymentFailedObjectList == null) { paymentFailedObjectList = new
		 * ArrayList<FailedPayment>();
		 * executionContext.put(Constant.REGULAR_DONATION_PAYMENT_FAILED_OBJECT_LIST
		 * , paymentFailedObjectList); }
		 * paymentFailedObjectList.add(failedPayment); LOGGER.trace(
		 * "Inside RegularDonationPaymentFailedTransformer UpdateObjectContext End."
		 * );
		 */
	}
	...
	public void beforeStep(StepExecution stepExecution) {
		...
		String batchNumber = Util
				.getGeneratedBatchNumber(MasterDataCodeConstants.WebDonation.WEB_DONATION_PAYMENT_FAILED
						.getCode());
		ServiceBatch serviceBatch = Util.createServiceBatch(batchNumber, stepExecution);

        serviceBatch.setCurrentJobName("regularDonationPaymentFailedJob");
		...
		try {
            ...
            LOGGER.error("Error occurred while creating batch  : RegularDonationPaymentFailedTransformer."
                        + batchExtManagementServiceFaultException.getMessage(), batchExtManagementServiceFaultException);
            throw new GivingBatchException(LOGGER,  "Batch Number"+batchNumber, batchExtManagementServiceFaultException);
        }
        catch(Exception exception)
