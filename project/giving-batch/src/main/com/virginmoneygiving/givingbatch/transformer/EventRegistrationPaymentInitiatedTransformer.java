 ...
 */
public class EventRegistrationPaymentInitiatedTransformer implements
		StepExecutionListener, ItemProcessor<Payment, PaymentInitiated> {
	
	/** Create instance of Logger to show Log information. */
	private static Logger LOGGER = Logger
			.getLogger(EventRegistrationPaymentInitiatedTransformer.class);
	...
	public PaymentInitiated process(Payment payment) throws GivingBatchException {
	    ...
	    if (LOGGER.isDebugEnabled()) {
    		LOGGER.debug("EventRegistrationPaymentInitiatedTransformer:process"
    				+ payment);
	    }
		...
		if (LOGGER.isDebugEnabled()) {
    		LOGGER
    				.debug("EventRegistrationPaymentInitiatedTransformer################################ BATCH NUMBER #######################:"
    						+ batchNumber);
		}

		PaymentInitiated paymentInitiated = new PaymentInitiated();
		Map<String, PaymentInitiated> map = new HashMap<String, PaymentInitiated>();
		if (payment != null) {
...
//						.getTransactionAmount());
		...
		batchEntity
				.setNextBatchStatus(MasterDataCodeConstants.EventRegistrationFee.EVENT_REG_FEE_PAYMENT_INITIATED
						.getCode());
		...
		try {
            ...
            LOGGER.error("Error occurred while creating batch entity : EventRegistrationPaymentInitiatedTransformer."
		...
		if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("EventRegistrationPaymentInitiatedTransformer:process END");
        }
	...
	private Map<String, PaymentInitiated> getSummaryMap() {
		...
		if (executionContext != null) {
			map = (Map<String, PaymentInitiated>) executionContext
	...
	private void updateObjectInContext(PaymentInitiated paymentInitiated) {
	    if (LOGGER.isTraceEnabled()) {
    		LOGGER
    				.trace("Inside EventRegistrationPaymentInitiatedTransformer UpdateObjectContext - START");
	    }
		if (executionContext != null) {
        ...
        if (LOGGER.isTraceEnabled()) {
            LOGGER
                    .trace("Inside EventRegistrationPaymentInitiatedTransformer UpdateObjectContext - END");
        }
	}
	...
	public String getGeneratedInvoiceNumber(PaymentService paymentService) {
		...
		if (LOGGER.isDebugEnabled()) {
		    LOGGER.debug("EventRegistrationPaymentInitiatedTransformer:generated sequence number is" + referenceNumber);
		}
	...
	public void beforeStep(StepExecution stepExecution) {
	    if (LOGGER.isDebugEnabled()) {
	        LOGGER.trace("EventRegistrationPaymentInitiatedTransformer: inside before step  execution context:" + stepExecution);
	    }
		...
		String batchNumber = Util
				.getGeneratedBatchNumber(MasterDataCodeConstants.EventRegistrationFee.EVENT_REG_FEE_PAYMENT_INITIATED
						.getCode());
		ServiceBatch serviceBatch = Util.createServiceBatch(batchNumber, stepExecution);
        serviceBatch.setCurrentJobName("eventRegistrationPaymentInitiatedJob");
        serviceBatch.setNextBatchName("eventRegistrationPaymentInitiatedJob");
		...
		try {
            ...
            LOGGER.error("Error occurred while creating batch  : EventRegistrationPaymentInitiatedTransformer."
                        + batchExtManagementServiceFaultException.getMessage(), batchExtManagementServiceFaultException);
            throw new GivingBatchException(LOGGER,  "Batch Number"+batchNumber, batchExtManagementServiceFaultException);
