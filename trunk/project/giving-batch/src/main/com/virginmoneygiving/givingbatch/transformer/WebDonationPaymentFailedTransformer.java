 ...
 * FailedPayment object.Before actual batch processing a unique batch number is created and 
 * after batch processing the batch outcome status is updated in batch table.
 * 
 * @author John Allen
 */
public class WebDonationPaymentFailedTransformer implements
        StepExecutionListener, ItemProcessor<Payment, FailedPayment> {
    
    /** Create instance of Logger to show Log information. */
    private static Logger LOGGER =
            Logger.getLogger(WebDonationPaymentFailedTransformer.class);
    ...
    public FailedPayment process(Payment payment) throws GivingBatchException {
        if(LOGGER.isDebugEnabled()) {
            LOGGER.debug("In WebDonationPaymentFailedTransformer" + payment);
        }
        ...
        Map<String, FailedPayment> map =
            new HashMap<String, FailedPayment>();
        if (payment != null) {
...
//                        .getTransactionAmount());
        ...
        if (map != null) {
            if (map.containsKey(cardType)) {
                ...
                paymentType2.setAmount(amount.add(failedPayment
        ...
        batchEntity.setNextBatchStatus(MasterDataCodeConstants.WebDonation.WEB_DONATION_PAYMENT_FAILED.getCode());
        ...
        try {
            ...
            LOGGER.error("Error occurred while creating batch Payment Entity : WebDonationPaymentFailedTransformer."
    ...
    public String getGeneratedInvoiceNumber(PaymentService paymentService) {
        String generateSequesnceNumber =
                paymentService.generateSequence("WEB_DONATION", "CREDIT_NOTE");
        String referenceNumber =
                generateSequenceHelper.prefixReferenceType("WEB_DONATION",
                        "CREDIT_NOTE", generateSequesnceNumber);
    ...
    private void updateObjectInContext(FailedPayment failedPayment) {
        if (executionContext != null) {
            ...
            if (summaryMap == null) {
                ...
                summaryMap
                        .put(failedPayment.getCardType(), failedPayment);
            }
            else {
                summaryMap
                        .put(failedPayment.getCardType(), failedPayment);
    ...
    public void beforeStep(StepExecution stepExecution) {
        ...
        serviceBatch.setCurrentJobName("webDonationPaymentFailedJob");
        ...
        try {
            ...
            LOGGER.error("Error occurred while creating batch  : WebDonationPaymentFailedTransformer."
                        + batchExtManagementServiceFaultException.getMessage(), batchExtManagementServiceFaultException);
            throw new GivingBatchException(LOGGER,  "Batch Number"+batchNumber, batchExtManagementServiceFaultException);
        }
        catch(Exception exception)
