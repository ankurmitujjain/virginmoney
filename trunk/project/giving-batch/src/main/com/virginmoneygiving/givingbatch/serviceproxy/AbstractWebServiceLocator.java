...
public abstract class AbstractWebServiceLocator {
    ...
    private static final Logger LOGGER =
            Logger.getLogger(AbstractWebServiceLocator.class);
    ...
    protected Service getWebService(final String serviceShortName)
        ...
        if (serviceClassName == null || serviceClassName.equals("")) {
            ...
            throw new GlisServiceException(LOGGER, "System property named "
                    + SERVICE_CLASSNAME_SUFFIX + " for service name="
                    + serviceShortName + " not set, cannot locate service");
        }
        LOGGER.debug("serviceShortName        =" + serviceShortName);
        LOGGER.debug("serviceClassName        =" + serviceClassName);
        LOGGER.debug("serviceName             =" + serviceName);
        LOGGER.debug("serviceURL              =" + getWebServiceURL());
        ...
        if (webServiceURL == null || webServiceURL.equals("")) {
            ...
            // TODO Need to Throw Exception Like the following way

            throw new GlisServiceException(LOGGER, "System property named "
                    + getWebServiceURL() + " for service name="
                    + serviceShortName + " not set, cannot locate service");
    ...
    private void loadServiceLocations() {
        if (!servicePropertiesLoaded) {
            ...
            try {
                ...
                LOGGER.info("serviceProperties location---"
                        + serviceProperties.getProperty("glis.classname"));
            }
            catch (IOException e) {
